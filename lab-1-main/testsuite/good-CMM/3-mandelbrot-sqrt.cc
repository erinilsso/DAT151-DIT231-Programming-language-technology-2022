...
// Auxiliary function to compute the smallest k with k² ≥ x
  ...
  )
{
  ...
  while (x2y2 <= 4.0 && iteration < maxiter) {
    // z ← z² + c
  ...
  return maxcolor + 1 - sqrt(iteration);
}

// Output the color values of an image of size height * width
// displaying the Mandelbrot set within the complex rectangle
//
void mandelbrot
  ( int maxcolor               // maximal color value
